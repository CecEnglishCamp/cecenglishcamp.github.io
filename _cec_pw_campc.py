...
NEW_CSS = """.pw-wrap{max-width:860px;margin:40px auto;padding:0 20px;display:flex;flex-direction:column;gap:16px;}
...
.pw-copy:hover{background:#047857;}
...
NEW_HTML = """<div class="pw-wrap">
  <div class="pw-prompt">
    <div class="pw-header">
      <img src="https://pub-a418b5aad0bd4c3fb41cf7159403fc12.r2.dev/robo_jump.gif" alt="Robo">
      <div>
        <div class="pw-header-t">AI 튜터 빅스와 영어 연습</div>
        <div class="pw-header-s">오늘 배운 내용을 ChatGPT에 붙여넣으세요</div>
      </div>
    </div>
...
for fpath in sorted(glob.glob(os.path.join(folder, '**', '*.html'), recursive=True)):
    ...
    # Skip if already has pw-wrap
    if 'pw-wrap' in content:
        continue

    if 'practice-wrap' not in content:
        continue
    ...
    # Remove old HTML
    content = re.sub(
        r'<!-- 프롬프트[^>]*-->.*?<div class="practice-wrap">.*?</div>\s*</div>\s*</div>',
        '',
        content,
        flags=re.DOTALL
    )
